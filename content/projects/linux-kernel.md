...
\* \*\*Module 2:\*\* A process-listing module that hooks into `<linux/sched.h>` to walk the kernel's internal linked list of every running process (`task\_struct`), mapping live process IDs and state codes directly from memory.
